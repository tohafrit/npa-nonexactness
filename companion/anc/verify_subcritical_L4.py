"""Float-free verifier: NPA level 4 is NOT exact at s=1/20 for the doubly-tilted
CHSH functional. Extends the certified ladder (level 1+AB at s=1/10, level 2 at
s=1/5) to level 4 -- concrete evidence that "no finite NPA level is exact" does not
stop at level 2.

Certificate (cert_subcrit_L4.json): a rational moment vector y over the level-4
word set (ScenarioL(4), 41x41 moment matrix) with
  (i)   Gamma_4(y) exactly positive semidefinite (rational LDL) and y_1 = 1
        -- a valid level-4 pseudo-moment (feasible for NPA level 4);
...
Hence c_4(1/40) >= B_s.y > c_Q(1/20): level 4 overshoots. QED.
...
def main():
    s4 = ScenarioL(4); N, idx, words = s4.N, s4.idx, s4.words
    def W(a, b): return words[(a, b)]
    al = sp.Rational(79, 80)  # 1 - s/2, s = 1/40
    d = json.load(open("cert_subcrit_L4.json"))
    yr = [Fraction(n, dd) for n, dd in zip(d["y_num"], d["y_den"])]
    # (i) feasibility at level 4
    ...
    beats = (roots_above == 0)
    print(f"(i) Gamma_4(y) exact PSD (41x41) + normalized: {feas}")
    print(f"(ii) objective (exact) = {float(obj):.12f}")
    print(f"     c_Q(1/40) largest sextic root ~ {float(max(r for r in sp.Poly(S,lam).nroots() if abs(sp.im(r))<1e-9).as_real_imag()[0]):.12f}")
    print(f"     sextic real roots above objective (Sturm): {roots_above}  (0 => obj > c_Q)")
    ok = feas and beats
    print("\nLEVEL 4 NOT EXACT AT s=1/40 VERIFIED (float-free)" if ok
          else "\nVERIFICATION FAILED")
