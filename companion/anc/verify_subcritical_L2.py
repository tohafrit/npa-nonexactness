"""Float-free verifier: NPA level 2 is NOT exact at s=1/5 for the doubly-tilted
CHSH functional. Extends verify_subcritical.py (which does level 1+AB at s=1/10)
one level up -- concrete evidence that the overshoot a_k>0 does not stop at the
almost-quantum level.

Certificate (cert_subcrit_L2.json): a rational moment vector y over the level-2
word set (ScenarioL(2), 13x13 moment matrix) with
  (i)   Gamma_2(y) exactly positive semidefinite (rational LDL) and y_1 = 1
        -- so y is a valid level-2 pseudo-moment (feasible for NPA level 2);
  (ii)  objective B_s.y (exact rational) strictly exceeds the quantum value
...
Hence c_2(1/5) >= B_s.y > c_Q(1/5): level 2 overshoots. QED.
...
def main():
    s2 = ScenarioL(2); N, idx, words = s2.N, s2.idx, s2.words
    def W(a, b): return words[(a, b)]
    al = sp.Rational(9, 10)  # 1 - s/2, s = 1/5
    d = json.load(open("cert_subcrit_L2.json"))
    yr = [Fraction(n, dd) for n, dd in zip(d["y_num"], d["y_den"])]
    # (i) feasibility at level 2
    ...
    beats = (roots_above == 0)
    print(f"(i) Gamma_2(y) exact PSD (13x13) + normalized: {feas}")
    print(f"(ii) objective (exact) = {float(obj):.10f}")
    print(f"     sextic real roots above objective (Sturm): {roots_above}  (0 => obj > c_Q)")
    ok = feas and beats
    print("\nLEVEL 2 NOT EXACT AT s=1/5 VERIFIED (float-free)" if ok
          else "\nVERIFICATION FAILED")
